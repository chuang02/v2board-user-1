...
import { title, notFoundPath } from '@/default'
import classNames from 'classnames/bind'
import moment from 'moment'

const TicketDetailPage: FC<IRouteComponentProps> = (props) => {
  const { match } = props
  const [userTicket, setUserTicket] = useState<API.User.TicketUnionItem>()
  ...
  const replyHandler = async () => {
    ...
    const ticketId = userTicket?.id as number
  ...
  const ticketFetch = async () => {
    ...
    setUserTicket(ticketResult.data)
      ...
      <div id="root">
        <div>
          <div className="block-content block-content-full bg-primary">
            <p className="font-size-lg font-w600 text-white mt-0 mb-0">{title}</p>
            <p className="text-white-75 mb-0">Support</p>
          </div>
          <div
            className="js-chat-messages block-content block-content-full text-wrap-break-word overflow-y-auto"
            style={{ height: bodyHeight }}
          >
            {userTicket?.message.map((messageItem: API.User.TicketMessageItem) => {
              return (
